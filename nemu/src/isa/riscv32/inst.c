...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, R(rd) = s->snpc; s->dnpc = (src1 + imm)-(src1 + imm)%2);
  ...
  INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh   , R, int32_t a = src1; int32_t b = src2; int64_t p = (int64_t)a * (int64_t)b; R(rd) = BITS(p, 63, 32));
  INSTPAT("0000001 ????? ????? 010 ????? 01100 11", mulhsu , R, int32_t a = src1; uint32_t b = src2; int64_t p = (int64_t)a * (uint64_t)b; R(rd) = BITS(p, 63, 32));
  INSTPAT("0000001 ????? ????? 011 ????? 01100 11", mulhu  , R, uint32_t a = src1; uint32_t b = src2; uint64_t p = (uint64_t)a * (uint64_t)b; R(rd) = BITS(p, 63, 32));
